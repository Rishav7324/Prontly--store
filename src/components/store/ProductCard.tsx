...
export function ProductCard({
  id, slug, title, price, priceRaw, compareAtPrice,
  category, images, rating, shortDescription, isFeatured,
}: ProductCardProps) {
  ...
  const discount = compareAtPrice && compareAtPrice > priceRaw
    ? Math.round(((compareAtPrice - priceRaw) / compareAtPrice) * 100) : null;

  return (
    <Link href={`/products/${slug || id}`} className="block group/card min-w-0">
      <div className="rounded-lg border border-border/60 bg-white overflow-hidden transition-all duration-200 hover:border-accent/30 hover:shadow-md flex flex-col h-full">
        {/* Image */}
        <div className="relative aspect-[3/4] bg-muted/40 overflow-hidden">
          ...
          <div className="absolute top-2 left-2 flex gap-1">
            {discount && (
              <span className="bg-emerald-500 text-white text-[9px] font-bold px-1.5 py-0.5 rounded">−{discount}%</span>
            )}
            {isFeatured && (
        ...
        <div className="p-3 flex-1 flex flex-col min-w-0">
          ...
          <h3 className="text-xs font-semibold leading-snug line-clamp-2 group-hover/card:text-accent transition-colors break-words">{title}</h3>
          {shortDescription && <p className="text-[11px] text-muted-foreground line-clamp-1 mt-0.5">{shortDescription}</p>}
          ...
          <div className="flex items-center justify-between mt-auto pt-2.5">
            <div className="min-w-0">
              ...
              {compareAtPrice && compareAtPrice > priceRaw && (
                <span className="text-[10px] text-muted-foreground line-through ml-1.5">₹{(compareAtPrice / 100).toLocaleString('en-IN')}</span>
              )}
            </div>
            <Button size="icon" className="h-7 w-7 rounded-md bg-zinc-900 hover:bg-zinc-800 text-white shrink-0"
