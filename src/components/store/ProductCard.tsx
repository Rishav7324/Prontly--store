...
export function ProductCard({ id, title, price, priceRaw, category, imageUrl, rating, sales }: ProductCardProps) {
  ...
  return (
    <Link href={`/products/${id}`} className="group block h-full">
      <Card className="h-full overflow-hidden bg-card transition-all duration-500 hover:border-primary/50 hover:shadow-[0_20px_50px_rgba(85,78,210,0.15)] border-white/5 rounded-3xl">
        <div className="relative aspect-[4/5] overflow-hidden">
          ...
          <button className="absolute left-4 top-4 h-9 w-9 rounded-xl bg-background/60 backdrop-blur-md flex items-center justify-center text-muted-foreground hover:text-destructive transition-all duration-300 opacity-0 group-hover:opacity-100 scale-90 group-hover:scale-100">
        ...
        <CardContent className="p-6 space-y-4">
          <div className="flex items-center justify-between">
            ...
            <span className="text-[10px] text-muted-foreground font-bold uppercase tracking-tighter">{sales} CREATORS</span>
          </div>
