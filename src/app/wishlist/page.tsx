    ...
    <div className="min-h-screen bg-background flex flex-col">
      ...
      <main className="flex-1 container mx-auto px-4 py-16">
        <header className="max-w-3xl mb-12">
          ...
          <h1 className="text-5xl font-bold font-headline leading-tight">Your Wishlist.</h1>
          <p className="text-xl text-muted-foreground mt-2">Saved assets waiting to elevate your next project.</p>
        </header>

        {loading ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
            {[...Array(4)].map((_, i) => (
              <div key={i} className="h-80 bg-muted animate-pulse rounded-[2rem]" />
            ))}
          </div>
        ) : products && products.length > 0 ? (
          <div className="space-y-12">
            <ProductGrid products={products} />
            <div className="p-10 rounded-[3rem] bg-primary/5 border border-primary/10 flex flex-col md:flex-row items-center justify-between gap-8 text-center md:text-left">
              <div className="space-y-2">
                <h3 className="text-2xl font-bold font-headline flex items-center justify-center md:justify-start gap-3">
                  <Sparkles className="h-6 w-6 text-primary" />
                  Ready to build?
                </h3>
                <p className="text-muted-foreground">Move your saved assets to the cart and start creating today.</p>
              </div>
          ...
          <div className="py-32 flex flex-col items-center justify-center text-center space-y-8 bg-muted/5 border-dashed border-2 border-white/5 rounded-[4rem]">
            ...
            <div className="space-y-2">
              <h2 className="text-3xl font-bold font-headline">Empty for now</h2>
              <p className="text-muted-foreground max-w-sm mx-auto">Explore our high-performance prompts and templates to fill your wishlist.</p>
            </div>
